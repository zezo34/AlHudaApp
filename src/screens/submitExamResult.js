...
export default function StudentExamScreen({ onBack }) {
  ...
  const handleSendAnswer = (exam) => {
    ...
    // إرسال الإجابة للـ Context لتظهر فوراً لولي الأمر
    if (submitExamResult) {
      submitExamResult({
        examTitle: exam.title,
        question: exam.question,
        studentName: 'أحمد محمد علي', // اسم الطالب
  ...
  return (
    <SafeAreaView style={styles.container}>
      {/* 🟢 شريط علوي أنيق يحمي الواجهة من نوتش الموبايل */}
        ...
        {exams.length === 0 ? (
          <View style={styles.emptyBox}>
            ...
            <Text style={styles.emptySubText}>
              أنت متفوق جداً! لم ينشر المعلم أي اختبار جديد بعد. استمتع بوقتك 🎈
            </Text>
          </View>
        ) : (
          exams.map((exam) => (
